...
USE_FOLDERS=0
MAX_DEPTH=3
DATE_TARGET="modify" # По умолчанию меняем только дату изменения
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        -h|--help) 
            ...
            echo "  -dc, --date-create Применить даты к 'созданию' (mtime + atime), а не только к изменению"
...
[[ "$DATE_TARGET" == "create" ]] && echo "Режим: Имитация даты создания (изменение mtime и atime)"
...
for (( i=1; i<=FILE_COUNT; i++ )); do
    ...
    if [ "$DATE_TARGET" == "create" ]; then
        # Имитируем дату создания: меняем и mtime, и atime
        touch -m -a -d "$RAND_DATE" "$FILE_PATH"
    else
        # Меняем только дату модификации (mtime)
        touch -m -d "$RAND_DATE" "$FILE_PATH"
    fi
    
    # Выводим прогресс раз в 100 файлов
    if (( i % 100 == 0 )); then
